...
def EvalNetwork(inputValues, Network):
    ...
    perceptron_and = Perceptron(np.array(Network[0]),1)
    and_result = perceptron_and.activate(inputValues)

    perceptron_or = Perceptron(np.array(Network[1]))
    or_result = perceptron_or.activate(inputValues)

    # Be sure your output value is a single number
    perceptron_or_minus_and = Perceptron(np.array(Network[2]))
    print "\n"+str(np.array([and_result,or_result]))
    result = perceptron_or_minus_and.activate(np.array([and_result,or_result]))
    return result
